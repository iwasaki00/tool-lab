...
import { registerWebMcp } from "../dev/integration/registerWebMcp";
...
import type { TestStartOptions } from "../dev/testing/TestBridge";
...
import { registerFrameworkUi, UiRegistry } from "../ui/UiRegistry";
...
import { DevComposition } from "../dev/DevComposition";

/** @internal Sample application host. Public consumers use framework/index.ts. */
export function runApplicationHost(): void {

const mobile = matchMedia("(pointer: coarse)").matches || navigator.maxTouchPoints > 0;
const gameComposition = createSampleGameComposition();
const uiRegistry = new UiRegistry();
registerFrameworkUi(uiRegistry);
gameComposition.registerUi(uiRegistry);
const lifecycle = new ApplicationLifecycle((stage) => console.info(`[APPLICATION] ${stage}`));
const devComposition = new DevComposition();
lifecycle.add(() => devComposition.dispose());
logFrameworkVersion();
...
let frameworkRuntime: FrameworkRuntime | undefined;
...
try {
  lifecycle.stage("FRAMEWORK_INITIALIZING");
  frameworkRuntime = bootstrapFramework(canvas, mobile);
  engine = frameworkRuntime.engine;
  lifecycle.stage("WORLD_LOADING");
  const initialScenario = gameComposition.getScenario(gameConfig, mobile);
  lifecycle.stage("FEATURES_INITIALIZING");
  ...
  lifecycle.stage("GAME_COMPOSING");
  ...
  const controls = createControls({
    ...
    movementSpeed: (settings) => {
      ...
      getLaboratory().api.getPlayer().setMovementSpeeds(settings);
    },
  ...
  const debugPanel = devComposition.createDebug({
    ...
    setSpeedMultiplier: (multiplier) => getLaboratory().api.getPlayer().setMovementSpeeds({ normalSpeed: movementSettings.normalSpeed * multiplier, shiftSpeed: movementSettings.shiftSpeed * multiplier }),
    guideMode: (mode) => { currentGuideMode = mode; getLaboratory().setMissionGuideMode(mode); },
    ...
    panelOpenChanged: (open) => getLaboratory().api.getPlayer().setInputEnabled(!open && gameSession?.snapshot().state === "PLAYING"),
  });
  ...
  function resetTestState(): void {
    getLaboratory().restartMission(citySettings); getLaboratory().api.getPlayer().setMovementSpeeds(movementSettings); applyGameRules();
  ...
  async function startConfiguredGame(sameSeeds: boolean): Promise<void> {
    ...
    await nextPaint();
    try {
      const canRestartComposition = sameSeeds && currentWorld === "city" && getLaboratory().cityStats()?.seed === citySettings.seed;
      if (canRestartComposition) getLaboratory().restartMission(citySettings);
      else rebuildWorld("city", citySettings, `${gameConfig.mode} / ${gameConfig.difficulty}`);
      getLaboratory().setPaused(true);
      ...
      lifecycle.error("GAME", error); console.error("GAME GENERATION FAILED", error); showGenerationError(error);
    }
  }
  ...
  function applyGameRules(): void {
    ...
    getLaboratory().api.getMap()?.setAutoExpansion(autoExpansion);
    getLaboratory().api.getMap()?.setChunkUnload(chunkUnload);
    document.body.classList.toggle("game-test-enabled", gameConfig.testMode);
  ...
  async function waitForNavigation(): Promise<void> {
    ...
    while (getLaboratory().api.getNavigation()?.stats().status === "BUILDING") {
      if (performance.now() - start > 7000) { getLaboratory().useNavigationFallback("NavMesh build timed out after 7 seconds"); break; }
      await delay(100);
    }
    const navigation = getLaboratory().api.getNavigation()?.stats();
    if (!navigation) throw new Error("Navigation feature is unavailable");
    if (navigation.status === "ERROR") getLaboratory().useNavigationFallback(navigation.error || "NavMesh build failed");
    const resolved = getLaboratory().api.getNavigation()?.stats();
    if (!resolved) throw new Error("Navigation feature is unavailable");
  ...
  function installMapControls(): void {
    ...
    expansionButton?.addEventListener("click", () => { autoExpansion = !autoExpansion; getLaboratory().api.getMap()?.setAutoExpansion(autoExpansion); syncButtons(); controls.showToast(`AUTO WORLD EXPANSION ${autoExpansion ? "ON" : "OFF"}`); });
    unloadButton?.addEventListener("click", () => { chunkUnload = !chunkUnload; getLaboratory().api.getMap()?.setChunkUnload(chunkUnload); syncButtons(); controls.showToast(`CHUNK UNLOAD ${chunkUnload ? "ON" : "OFF"}`); });
    document.querySelector("#export-map-button")?.addEventListener("click", () => { const map = getLaboratory().api.getMap()?.saveMap(); if (!map) return; downloadMap(map); controls.showToast(`MAP EXPORTED — ${map.chunks.length} CHUNKS`); });
  ...
  function installVisualControls(): void {
    ...
    if (environment) { environment.value = environmentPreset; environment.addEventListener("change", () => { environmentPreset = environment.value as EnvironmentPreset; currentTime = environmentPreset === "NIGHT" ? "night" : "day"; getLaboratory().api.getVisual().setEnvironment(environmentPreset); controls.setMode(currentTime); controls.showToast(`ENVIRONMENT ${environmentPreset}`); }); }
    if (quality) { quality.value = visualQuality; quality.addEventListener("change", () => { visualQuality = quality.value as VisualQuality; getLaboratory().api.getVisual().setQuality(visualQuality); controls.showToast(`VISUAL QUALITY ${visualQuality}`); }); }
  ...
  async function applyImportedMap(value: unknown): Promise<void> {
    ...
    currentWorld = "city"; citySettings = { ...citySettings, seed: Number(candidate.seed) }; saveCitySettings(citySettings); rebuildWorld("city", citySettings, "保存Mapを読み込みました"); const map = await getLaboratory().api.loadMap(value); autoExpansion = map.metadata.autoExpansion; chunkUnload = map.metadata.chunkUnload; renderMapBrowser();
  }
  ...
  devComposition.installTestBridge({ laboratory: getLaboratory, canvas, config: () => ({ ...gameConfig }), session: () => gameSession?.snapshot(), startGame: startTestGame });
  ...
  lifecycle.error(lifecycle.status() === "FEATURES_INITIALIZING" ? "FEATURE" : "FRAMEWORK", error);
...
window.addEventListener("beforeunload", () => {
  detachMobileControls();
  try { laboratory?.disposeWorld(); laboratory?.scene.dispose(); } catch (error) { console.warn("SCENE DISPOSE WARNING", error); }
  frameworkRuntime?.dispose();
  uiRegistry.dispose();
  lifecycle.dispose();
}, { once: true });
