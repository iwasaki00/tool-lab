...
export function installTestBridge(options: TestBridgeOptions): () => void {
  const query = new URLSearchParams(location.search);
  if (!import.meta.env.DEV || (!query.has("e2e") && !query.has("demo"))) return () => undefined;
  ...
  return () => { delete window.__SPACE_LAB_TEST__; delete document.documentElement.dataset.testBridge; document.querySelector("#e2e-demo-status")?.remove(); };
}
