...
import { validateMission, type MissionValidation } from "../../gameplay/MissionValidator";
import { MissionGuideManager } from "../../gameplay/MissionGuideManager";
import { ObjectiveManager } from "../../gameplay/ObjectiveManager";
import { MissionRuntime } from "../../gameplay/MissionRuntime";
...
import { DiscoveryManager } from "../../gameplay/DiscoveryManager";
import { DebugTestManager } from "../../debug/DebugTestManager";
import { FRAMEWORK_EVENT, type FrameworkEventMap } from "../../contracts/FrameworkEvents";
import type { GameplayCallbacks, LaboratoryScenario, ScenarioPolicy } from "../../contracts/ScenarioContracts";

export type { GameplayCallbacks } from "../../contracts/ScenarioContracts";
export type DemoScenarioPolicy = ScenarioPolicy;
export type DemoScenario = LaboratoryScenario;

const SAMPLE_MISSION_TEMPLATES: MissionTemplateSource = {
  get: (type) => MISSION_TEMPLATES[type],
  instantiate: (type, difficulty, targets) => instantiateTemplate(MISSION_TEMPLATES[type], difficulty, targets),
};
...
export function createDemoScenario(
  ...
  scenarioPolicy: DemoScenarioPolicy = { enemyCount: 2, npcCount: 3, discoveryEnabled: false },
  ...
  for (let retry = 0; retry < 10; retry += 1) {
    plan = new MissionGenerator(registry, placement, citySeed, missionSeed, SAMPLE_MISSION_TEMPLATES).generate(spawn, missionSite.id, missionType, missionDifficulty, retry);
  ...
  const runtime = new MissionRuntime(plan, objectives, (state) => {
    ...
    if (state.result && !completionDelivered) {
      completionDelivered = true;
      frameworkEvents?.emit(FRAMEWORK_EVENT.MISSION_COMPLETED, { missionType: state.result.type, difficulty: state.result.difficulty, seed: state.result.seed, clearTimeSeconds: state.result.clearTimeSeconds });
      callbacks.onMissionComplete(state.result);
    }
  });
  ...
  const characterCounts = createSemanticSpawnPoints(placement, registry, spawn, scenarioPolicy);
  ...
  const discovery = new DiscoveryManager(ctx.scene, camera, registry, scenarioPolicy.discoveryEnabled, (snapshot, areaId) => {
    callbacks.onDiscovery?.(snapshot);
    if (areaId) frameworkEvents?.emit(FRAMEWORK_EVENT.AREA_DISCOVERED, { areaId, label: snapshot.lastLabel });
  });
...
function createSemanticSpawnPoints(placement: GamePlacementManager, registry: WorldRegistry, start: Vector3, policy: DemoScenarioPolicy): { enemyCount: number; npcCount: number } {
  const { enemyCount, npcCount } = policy;
