...
export class GameSession {
  ...
  private readonly mode: IGameMode;

  constructor(readonly config: GameConfig, private readonly onChange: (snapshot: GameSessionSnapshot) => void, mode?: IGameMode) { this.mode = mode ?? getGameMode(config.mode); }
  ...
  caught(): void { if (this.state !== "PLAYING") return; this.caughtCount += 1; if (this.mode.failOnCaught && !this.config.testMode) this.fail("敵に捕まりました"); else this.publish(); }
  updateDiscovery(value: DiscoverySnapshot): void { this.discovery = value; if (this.mode.isDiscoveryComplete(value)) this.complete(); else this.publish(); }
  ...
  private finish(status: "COMPLETE" | "FAILED", reason?: string): void {
    ...
    this.result = {
      ...
      clearTimeSeconds: seconds, score: this.scores.calculate(this.mode, { seconds, detections: this.detections, discovered: this.discovery.discovered, landmarkFound: this.discovery.landmarkFound, complete: status === "COMPLETE" }) + this.debugScoreBonus,
