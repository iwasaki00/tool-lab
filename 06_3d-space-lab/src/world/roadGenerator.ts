...
export function createRoad(ctx: ObjectContext, options: RoadOptions): Mesh {
  const { position, width, length, rotation = 0, markings = true } = options;
  const root = new Mesh("city-road", ctx.scene);
  root.position.copyFrom(position); root.rotation.y = rotation;
  const asphalt = createMaterial(ctx.scene, "city-asphalt", new Color3(.075, .09, .105), .04);
  const concrete = createMaterial(ctx.scene, "city-sidewalk", new Color3(.45, .47, .46), .08);
  ...
  [-1, 1].forEach((side) => {
    const sidewalk = createSidewalk(ctx, width, length, side);
...
export function createSidewalk(ctx: ObjectContext, roadWidth: number, length: number, side: number): Mesh {
  const sidewalk = MeshBuilder.CreateBox("city-sidewalk", { width: 1.8, depth: length, height: .22 }, ctx.scene);
  sidewalk.position.set(side * (roadWidth / 2 + .9), .11, 0);
