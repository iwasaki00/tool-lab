...
export function createBench(ctx: ObjectContext, position: Vector3, rotation = 0): Mesh {
  ...
  const wood = createMaterial(ctx.scene, "bench-wood", new Color3(.48, .25, .1));
  const metal = createMaterial(ctx.scene, "bench-metal", new Color3(.12, .14, .15));
  const add = (name: string, width: number, height: number, depth: number, x: number, y: number, z: number, material = wood) => {
    ...
    mesh.position.set(x, y, z); mesh.parent = root; mesh.material = material; mesh.checkCollisions = true;
    return mesh;
  };
